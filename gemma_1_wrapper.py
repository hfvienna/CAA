...
#    tokenize_gemma_1_chat,
    tokenize_gemma_1_base,
    ADD_FROM_POS_BASE,
    ADD_FROM_POS_CHAT,
)
...
class Gemma1Wrapper:
    def __init__(
        ...
        hf_token: str,
        size: str = "2b",
        # Manual override hfvienna
        use_chat: bool = False,
        override_model_weights_path: Optional[str] = None,
        model_type: str = "gemma_1"
    ):
        self.device = "cuda" if t.cuda.is_available() else "cpu"
        self.use_chat = use_chat
        print(f"Use chat: {use_chat}")
        self.model_name_path = get_model_path(size, not use_chat, model_type)
        print(f"Model name path: {self.model_name_path}")
    ...
    def generate_text(self, user_input: str, model_output: Optional[str] = None, system_prompt: Optional[str] = None, max_new_tokens: int = 50) -> str:
        if self.use_chat:
            tokens = tokenize_gemma_1_chat(
                tokenizer=self.tokenizer, user_input=user_input, model_output=model_output, system_prompt=system_prompt
            )
        else:
            tokens = tokenize_gemma_1_base(tokenizer=self.tokenizer, user_input=user_input, model_output=model_output)
